...
function woo_exclude_categories_homepage ( $query ) {
	...
	// Homepage logic.
	if ( $query->is_home() && $query->is_main_query() && ( count( $excluded_cats ) > 0 ) ) {
...
if (!function_exists( 'woo_subscribe_connect')) {
	function woo_subscribe_connect($widget = 'false', $title = '', $form = '', $social = '', $contact_template = 'false') {
		...
		$settings = array(
						...
						'connect_rss' => '',
						'connect_googleplus' => '',
						'contact_subscribe_and_connect' => 'false'
						);
...
?>
	<?php if ( ( $settings[ 'connect' ] == "true" OR $widget == 'true' ) || ( is_page_template('template-contact.php') && $settings[ 'contact_subscribe_and_connect' ] == "true" ) ) : ?>
