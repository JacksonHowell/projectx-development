...
- Homepage query filter for the content area
...
$default_google_fonts = array( 'Muli' );
...
if ( ! function_exists( 'woo_custom_styling' ) ) {
	function woo_custom_styling() {
		...
		if ( $settings['header_bg_color'] != '' ) {
			$output .= '#header { background-color: ' . $settings['header_bg_color'] . ' !important; }' . "\n";
			$output .= '#header .search_main .searchform { border: none; }' . "\n";
		}
		
		if ( $settings['header_font_color'] != '' ) {
			$output .= '#header #navigation .nav a { color: ' . $settings['header_font_color'] . ' !important; text-shadow: none; }' . "\n";
		}
		
		if ( $settings['header_hover_color'] != '' ) {
			$output .= '#header #navigation .nav a:hover, #header #navigation .nav li.current_page_item a, #header #navigation .nav li.current_page_parent a, #header #navigation .nav li.current-menu-ancestor a, #header #navigation .nav li.current-cat a,  #header #navigation .nav li:hover > a, #header #navigation .nav li ul { background-color: ' . $settings['header_hover_color'] . ' !important; }' . "\n";
		}
		
		if ( $settings['header_active_color'] != '' ) {
			$output .= '#header #navigation .nav a:hover, #header #navigation .nav li.current_page_item a, #header #navigation .nav li.current_page_parent a, #header #navigation .nav li.current-menu-ancestor a, #header #navigation .nav li.current-cat a,  #header #navigation .nav li:hover > a  { color: ' . $settings['header_active_color'] . ' !important; }' . "\n";
		}
		
		if ( $settings['header_plain'] == 'true' ) {
			$output .= '#header:before { display: none!important; }' . "\n";
		}
		
		if ( $settings['body_color'] != '' ) {
			$output .= 'body { background: ' . $settings['body_color'] . ' !important; }' . "\n";
			$output .= '#header:before { color: ' . $settings['body_color'] . ' !important; }' . "\n";
		}
		...
		if ( $settings['button_color'] != '' ) {
			$output .= '.button, a.button, #commentform #submit, #contact-page .submit { background: ' . $settings['button_color'] . ' !important; border-color: ' . $settings['button_color'] . ' !important; }' . "\n";
			$output .= '.button, a.button:hover, a.button.hover, a.button.active, #commentform #submit:hover, #contact-page .submit:hover { background: ' . $settings['button_color'] . ' !important; opacity: 0.9; }' . "\n";
		}
	...
	function woo_custom_typography() {
		...
		if ( isset( $woo_options['woo_typography'] ) && $woo_options['woo_typography'] == 'true' ) {
			...
			if ( isset( $woo_options['woo_font_nav'] ) && $woo_options['woo_font_nav'] )
				$output .= '#navigation .nav li a { '.woo_generate_font_css($woo_options['woo_font_nav'], '1.4').' }' . "\n";	
...
if ( ! function_exists( 'woo_homepage_content_query_filter' ) ) {
function woo_homepage_content_query_filter ( $query ) {
	if ( $query->is_home() && $query->is_main_query() && ! $query->is_admin ) {
		...
		if ( $settings['content_area_content'] == 'page' ) {
	        $page_id = $settings['content_area_page'];
	       
	        $query->set( 'post_type', 'page' );
	        $query->set( 'page_id', $page_id );
	        $query->set( 'posts_per_page', 1 );
	    } else {
	        $number_of_features = $settings['content_area_entries'];
	...
	return $query;
} // End woo_homepage_content_query_filter()
}

add_filter( 'pre_get_posts', 'woo_homepage_content_query_filter' );
