...
- Ensure the pagination of the posts on the homepage is correct (WordPress 3.4+)
- Register WP Menus
- Page navigation
- Post Meta
- Subscribe & Connect
...
add_filter( 'pre_get_posts', 'woo_filter_homepage_posts_query' );

function woo_filter_homepage_posts_query ( $query ) {
	global $woo_options;

	if ( $query->is_home() && $query->is_main_query() ) {
		$query->set( 'post_type', array( 'post' ) );
		$query->set( 'posts_per_page', intval( $woo_options['woo_blog_number_of_posts'] ) );
		$query->set( 'suppress_filters', false );
	}

	return $query;
} // End woo_filter_homepage_posts_query()
	...
	<div id="connect">
		...
		<div <?php if ( $related_posts != '' ) echo 'class="col-left"'; ?>>
			...
			<div class="social<?php if ( $related_posts == '' AND $woo_options[ 'woo_connect_newsletter_id' ] != "" ) echo ' fr'; ?>">
		   		...
		   		<a href="<?php echo $woo_options[ 'woo_connect_googleplus' ]; ?>" class="google+"><img src="<?php echo get_template_directory_uri(); ?>/images/ico-social-googleplus.png" title="<?php esc_attr_e( 'Connect on Google+', 'woothemes' ); ?>" alt=""/></a>
